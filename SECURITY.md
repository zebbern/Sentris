...
- Alternative: Email `z_r@tuta.com` with a clear subject (e.g., `Vulnerability report: <area>`).
...
- **Timing-safe token comparison**: Internal service authentication in `auth.guard.ts` uses `crypto.timingSafeEqual` to prevent timing side-channel attacks when comparing bearer tokens.
- **Rate limiting**: `@nestjs/throttler` with Redis storage enforces per-controller request limits (e.g., secrets 30/min, API keys 20/min, inbound webhooks 60/min) to mitigate brute-force and abuse.
