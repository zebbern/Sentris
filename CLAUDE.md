...
It can be spawned agents for specific tasks, but don't let them run wild. Review their output and verify it against the real code/real run before accepting it.

## Presentation & UX

Build for flexible, context-aware presentation rather than fixed dimensions or one-size layouts.

- Prefer structures that adapt to the surface (Embeds and components, CLI/TUI output, docs, or a future web UI) instead of hard-coded widths, lengths, or one-device assumptions.
...
- Keep consistent design accross the platform and same With buttons/placements not limited to only this but as an example scenario as why this should be done: User uses "+ New" button in the top right in the last 3 tabs the user has been in then suddenly the "+ New" button is at the bottom of the page. That may confuse the user which we should try to avoid. 
