 ...
 */
export const humanInputRequests = pgTable(
  ...
  (table) => ({
    runIdx: index('human_input_requests_run_id_idx').on(table.runId),
    orgIdx: index('human_input_requests_organization_id_idx').on(table.organizationId),
    statusIdx: index('human_input_requests_status_idx').on(table.status),
  }),
);
