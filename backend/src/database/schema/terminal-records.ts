...
export const workflowTerminalRecordsTable = pgTable(
  'workflow_terminal_records',
  {
  ...
  (table) => ({
    runOrgCreatedAtIdx: index('terminal_records_run_org_created_at_idx').on(
      table.runId,
      table.organizationId,
      table.createdAt,
    ),
    orgIdx: index('terminal_records_org_idx').on(table.organizationId),
  }),
);
