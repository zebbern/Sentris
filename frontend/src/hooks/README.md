...
| `useWorkflowQueries.ts`    | `useWorkflowsList`, `useWorkflowsSummary`, `useWorkflow`, `useWorkflowRuntimeInputs`, `useDeleteWorkflow`, `useWorkflowVersions` — Workflow metadata, detail, inputs, version history + delete mutation |
...
- **useCopyToClipboard.ts** — Centralized clipboard operations with toast feedback and auto-reset after 2 seconds
- **useNotifications.ts** — Subscribes to execution lifecycle events, dispatches browser and store notifications based on user preferences
- **useNotificationPermission.ts** — Manages browser Notification API permission state with graceful degradation
