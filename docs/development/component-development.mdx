...
description: "Build custom security components for Sentris Flow"
---

This guide covers everything you need to build components for Sentris Flow.
...
├── core/            # Core utilities (http-request, file-loader, text-block)
├── ai/              # AI components (llm, agents, opencode)
├── notification/    # Notifications (slack, discord, jira)
├── manual-action/   # Human-in-the-loop (approvals, forms)
├── github/          # GitHub integrations (connection-provider, remove-org-membership)
├── it-automation/   # IT ops (atlassian-offboarding, okta, google-workspace)
├── dev/             # Development utilities (mock-agent)
└── test/            # Test utilities (sleep-parallel, live-event-heartbeat)
```
...
Examples:
  sentris.dnsx.run       # Security tool
  ...
  notification.run-report-discord # Run export bundles to Discord
```

**Run inspector vs workflow outputs:** The run inspector tabs (Events, Logs, Findings, Artifacts, etc.) are post-run views over stored data. Workflow notification components only send data wired in the graph—or, for `core.notification.run-report-discord`, bundles fetched from internal run APIs at execution time. Wire **Run after** from the final upstream node so the scheduler waits for scanners; set **Webhook URL** via secret (do not wire report outputs to the webhook port).
...
```typescript
import { inputs, port } from '@sentris/component-sdk';
...
```typescript
import { generateFindingHash } from '@sentris/component-sdk';
...
runner: {
  kind: 'docker',
  image: 'ghcr.io/zebbern/subfinder:latest',
...
The `bun run test:e2e*` commands resolve the active local instance and load `e2e-tests/.env.e2e` automatically. Use `SENTRIS_E2E_ENV_FILE=/path/to/custom.env` when you intentionally want a different env file.
...
export default defineComponent({
  id: 'sentris.dnsx.scan',
  label: 'DNSX Scanner',
  category: 'security',
  runner: {
    kind: 'docker',
    image: 'ghcr.io/zebbern/dnsx:latest',
    entrypoint: 'sh',
    command: ['-c', 'dnsx "$@"', '--'],
    network: 'bridge',
  },
