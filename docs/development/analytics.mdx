...
Sentris Flow can use PostHog for server product events plus browser analytics
and session recording. Both paths are opt-in runtime configuration, so local
clones and generic release images collect nothing when the values are absent.
...
|----------|-------------|
| `POSTHOG_API_KEY` | Backend project API key |
| `POSTHOG_HOST` | Backend PostHog host |
| `DISABLE_ANALYTICS` | Explicit backend kill switch |
| `VITE_PUBLIC_POSTHOG_KEY` | Project API key |
| `VITE_PUBLIC_POSTHOG_HOST` | PostHog host (e.g., `https://us.i.posthog.com`) |
| `VITE_DISABLE_ANALYTICS` | Explicit browser kill switch |

See `backend/.env.example` and `frontend/.env.example` for templates. Each path
is disabled unless its key and host are both set, and either kill switch takes
precedence. Values are injected when containers start; they are not embedded in
published images.
...
`frontend/src/components/AnalyticsWrapper.tsx` reads the validated runtime
configuration, lazily loads the PostHog client, initializes it, and mounts
`PostHogProvider` only when both public variables are present.
