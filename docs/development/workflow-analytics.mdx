...
| Workflow context | `sentris.*` | Automatic metadata from the workflow |
...
<Warning>
  If you don't see `sentris.*` fields in Available Fields after indexing, refresh the index pattern field list in Dashboards Management.
</Warning>
