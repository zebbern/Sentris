...
| **PM2**            | 6.x                      | Bundled for `bun run dev`; global optional | Installed as a repo devDependency. Use `bun run pm2 -- ...` for manual PM2 commands. |
| **just**           | Latest                   | Optional     | [github.com/casey/just](https://github.com/casey/just)      |
| **Git Bash / WSL** | —                        | Windows only | Required for `just` recipes; not required for `bun run dev` / `bun run dev:stop` |

> **Windows note:** The `just` command and several specialized `bun` scripts use bash. Native PowerShell can use the cross-platform `bun run dev` and `bun run dev:stop` scripts.
...
### Method 2: Using bun scripts (cross-platform)
...
These scripts use the local PM2 package from `node_modules`, respect `SENTRIS_INSTANCE` and `.sentris-instance`, initialize or repair `.instances/instance-N/{backend,worker,frontend}.env` before startup, and do not require a global `pm2` executable. For example, `SENTRIS_INSTANCE=2 bun run dev` starts `sentris-frontend-2`, `sentris-backend-2`, and `sentris-worker-2`; `SENTRIS_INSTANCE=2 bun run dev status` checks PM2, Docker, and runtime health endpoints without starting services; `SENTRIS_INSTANCE=2 bun run dev stop` stops only those PM2 apps and leaves shared Docker infra running.
...
# 5. Start application via repo-local PM2 (manual instance 0 example; replace 0 with the active instance if needed)
...
```bash
bun run pm2 -- delete sentris-frontend-0 sentris-backend-0 sentris-worker-0
...
`bun run dev`, `bun run dev:stop`, and `bun run pm2 -- ...` use the repo-local PM2 package and should work after `bun install`. Prefer the wrapper for manual PM2 commands:
...
# Check PM2 processes
node scripts/pm2-cli.js jlist 2>/dev/null | grep -o '"name":"sentris-[^"]*","status":"[^"]*"' || echo "PM2 not running"
```
...
```bash
# Frontend only
bun run pm2 -- restart sentris-frontend-0

# Backend only
bun run pm2 -- restart sentris-backend-0

# Worker only
bun run pm2 -- restart sentris-worker-0
...
# Specific process
bun run pm2 -- logs sentris-backend-0 --lines 50
...
| Symptom                             | Action                                                                                                                      |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| PM2 process shows `errored`         | Run `bun run pm2 -- logs <name> --lines 100` to diagnose, then `bun run pm2 -- restart <name>`                              |
...
| Frontend shows blank page           | Check `bun run pm2 -- logs sentris-frontend-0`. Common cause: missing `frontend/.env`                                       |
