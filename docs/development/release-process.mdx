...
2. **Tags** images with:
   - Version tag: `ghcr.io/zebbern/studio-{service}:{version}`
   - Latest tag: `ghcr.io/zebbern/studio-{service}:latest`
...
| Service | Image |
|---------|-------|
| Backend | `ghcr.io/zebbern/studio-backend:{version}` |
| Worker | `ghcr.io/zebbern/studio-worker:{version}` |
| Frontend | `ghcr.io/zebbern/studio-frontend:{version}` |
...
docker pull ghcr.io/zebbern/studio-backend:1.0.0

# Pull latest
docker pull ghcr.io/zebbern/studio-backend:latest
```
...
| `@sentris/studio-worker` | v0.1.0 | No (private) |
