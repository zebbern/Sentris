...
- **Complete Capability Browsing**: Inspect saved tools, exact resources, resource templates, and prompts from the latest validated discovery
...
The **MCP Library** page stores the latest complete discovery catalog for each saved server.
Use **View capabilities** to inspect its Tools, Resources, and Prompts. Resource templates
are labeled separately from exact resource URIs, and prompt arguments identify required
values with an asterisk. Rediscovery updates this catalog and the tool enablement rows in
one database transaction; opening the dialog does not start a live MCP runtime.

Use **Preview** to read an exact resource, expand and read a resource template, or retrieve
a prompt with its arguments. Preview is an explicit user action: only then does Sentris
acquire the selected saved server through the canonical worker-owned runtime. Text, binary
metadata, embedded resource content, and prompt message roles remain visible alongside the
bounded raw MCP response.
...
<Note>
  Without a `GITHUB_REGISTRY_TOKEN`, sync uses GitHub's unauthenticated rate limit of 60 requests
  per hour. This is sufficient for the daily sync schedule, but may be insufficient if you trigger
  manual syncs frequently.
</Note>
