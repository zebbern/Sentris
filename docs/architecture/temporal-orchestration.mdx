...
description: "How Sentris Flow uses Temporal for workflow and activity management"
---

# Temporal Orchestration

Sentris Flow uses [Temporal.io](https://temporal.io) as its core orchestration engine. Temporal ensures that workflows are durable, retriable, and scalable.
...
Sentris uses a specialized worker architecture to handle different types of workloads.

### Default Worker
- **Task Queue**: `sentris-default`
- **Responsibilities**: 
...
2.  **Temporal Server** places a "Workflow Task" on the `sentris-default` queue.
...
## Scaling
Sentris workers are horizontally scalable. Multiple workers can poll the same task queue, and Temporal will automatically load-balance tasks between them. Workers can be deployed as separate Docker containers or Kubernetes pods.
