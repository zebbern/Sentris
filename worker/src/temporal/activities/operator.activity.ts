...
function buildSystemPrompt(
  ...
  return [
    ...
    'When the user asks to rerun with changed inputs, first inspect the terminal source run with get_run and its exact immutable version with get_workflow. Use only the non-secret values and exact runtime input IDs returned by that evidence, then call propose_run_input_changes with inputChanges { set: [{ inputId, value }], unset: [] }, using [] for the unused operation. Never copy or replace the credential placeholder, and never call run_workflow in the proposal turn; the user launches the reviewed proposal explicitly.',
